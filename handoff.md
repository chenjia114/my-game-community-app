...
## 2026-04-21 首页 Pencil 对齐阶段记录
- 已完成首页第一阶段 Pencil 对齐梳理：当前改动集中在 `app/(tabs)/index.tsx`、`components/NewsCard.tsx`、`components/PostCard.tsx`、`constants/theme.ts`、`app/(tabs)/_layout.tsx`。
- 这批改动的目标是把 Web 首页视觉调整为更轻的浅色产品风格，保持首页现有数据来源、跳转逻辑、下拉刷新、信息流混排逻辑不变。
- 当前已落地的视觉点包括：顶部搜索入口卡片、今日资讯横向卡片区、混合信息流标题与留白、帖子/资讯卡片圆角和阴影减轻、Tab 和 Header 统一成浅色风格。
- 下一步必须做的事：先本地跑 Web 验证首页视觉和交互是否正常，再决定是否还要继续微调，最后再部署 Vercel 预览。


## 当前首页 Pencil 对齐与 Web 验证进展（2026-04-21 本次继续验收）
### 已完成
- 已再次运行 `npm run verify`，当前通过；仍只有旧的 4 条 lint warning，没有新增 error。
- 已在本地 Web 继续验收首页关键路径：首页可正常渲染，顶部包含‘搜索帖子标题’、‘今日资讯’、‘混合信息流’；点击搜索入口后可正常跳转到 `/search`。
- `/search` 页面当前可正常打开，控制台为 `0 errors, 1 warnings`，说明首页到搜索页这条主路径在 Web 端没有被本轮首页视觉改造破坏。

### 当前下一步
- 继续补完首页返回链路与主要可见区块的视觉确认，然后执行 Vercel 预览部署并返回地址。

## 当前底部导航发布按钮精简进展（2026-04-21）
### 已完成
- 已把底部导航中间项的“发布”文字去掉，只保留加号圆形按钮，页面入口与顶部标题保持不变。
- 这次改动落在 `app/(tabs)/_layout.tsx`，通过 `tabBarShowLabel: false` 让中间 Tab 不再显示文字标签。

### 当前下一步
- 本地确认底部导航显示正常后，重新部署到 Vercel 并返回新的预览地址。

## 当前预览环境变量缺失回退处理进展（2026-04-21）
### 已完成
- 已确认这次不是首页 UI 改动把业务逻辑改坏，而是 Vercel `preview` 环境变量当前为空，导致预览部署里拿不到 `EXPO_PUBLIC_SUPABASE_URL`、`SUPABASE_SERVICE_ROLE_KEY`、`TAVILY_API_KEY` 等关键变量。
- 由于当前项目未连接可识别的本地 Git 分支，CLI 在补 `preview` 变量时还会继续要求指定分支，短时间内不适合直接在这台机器上把 preview 环境补齐。
- 已按用户选择改走 `production` 部署；生产环境变量仍然存在，重新部署后正式别名已重新指向新版本。

### 当前下一步
- 让用户直接在正式地址复测首页资讯加载、发帖页、管理台是否恢复正常。
- 如果后面仍需要可用的预览环境，再单独到 Vercel 后台把 preview 变量按分支或全局补齐。

## 当前首页底部导航与标题文案微调进展（2026-04-21）
### 已完成
- 已把首页底部导航“首页 / 我的”的图标和文字整体上移一点，并增加底栏可用高度，避免 Web 端底部标签被截断。
- 已把首页区块标题“混合信息流”改成“大家在聊”，其余信息流逻辑保持不变。

### 当前下一步
- 如果用户还觉得底部导航位置不够舒服，再继续细调图标尺寸、标签间距和底栏高度。

## 当前资讯图文严格对应改造进展（2026-04-22）
### 已完成
- 已停止使用 Tavily search 顶层 `images[]` 给资讯按索引配图，避免继续出现标题和图片串位。
- 当前抓取改成两段：先搜索候选资讯，再按文章 URL 逐条调用 Tavily extract 提取该文章自己的图片；只有确认拿到图片的文章才会入库，没有图片的资讯会直接跳过。
- 每一次 Tavily search / extract 请求现在都会进入同一个每日额度控制，继续受 `20 次/天` 上限约束，不再只统计搜索请求。
- `scripts/crawl-news.mjs` 已同步为同一策略，避免后台抓取和脚本抓取再次分叉。
- 已重新运行 `npm run verify`，当前通过；没有新增 error，仅有旧 warning 和本轮新增的 `Array<T>` lint warning。

### 当前下一步
- 直接部署到 Vercel 正式环境。
- 在线上管理后台重新抓取资讯，重点确认：入库资讯都有图、无图资讯被跳过、首页图文对应关系恢复正常。
